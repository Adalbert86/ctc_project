...
import { HelpBlock, Grid, Navbar, Jumbotron, Button, Glyphicon, ProgressBar, Row, Col, Table, Form, FormGroup, FormControl, ControlLabel, Checkbox  } from 'react-bootstrap';
...
class NewCustomerComponent extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = {name: '', password: '', email: '', submitDisabled: true, redirectToNewPage: false };
...
// })();
  ...
  handleSubmit(event) {
	...
	if (this.isFormValidReadyToSend()) {
		...
		// submit
		var jsonPayload = JSON.stringify({name: this.state.name, email: this.state.email, password: this.state.password});
  ...
  isFormValidReadyToSend() {
  	...
  	return (this.getValidationState() == 'success' && this.state.name.length > 0 && this.state.email.length > 0)
  }
...
<Form horizontal onSubmit={this.handleSubmit}>
  ...
  <FormGroup controlId="formHorizontalName">
    ...
    <Col sm={10}>
      <FormControl type="text" placeholder="First name and last name" value={this.state.name} onChange={this.handleNameChange} />
    </Col>
  </FormGroup>
